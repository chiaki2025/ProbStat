...
\\ Roll the die and observe the landing value. $X$ is the random variable defined as the value observed:
$$S_{1}=\{1,2,3,4,5,6\}$$
...
$$S_{3}=\{z\;|\;0< z<1\}$$
The random variable $X$ can take one of the values $x_{1}=1,x_{2}=2,\cdots,x_{6}=6$, and similarly with $Y$ and $Z$ can take one of their own values in their sample spaces $S_{2}$, $S_{3}$ respectively. Now we interested in classifying $2$ types of sample space and their random variables.
\begin{definition}
...
\begin{definition}
	...
	\end{equation*}
\end{definition}
You can find out that the pdf of coin tossing experiment above is:
...
\begin{definition}
	The \textbf{cumulative distribution function} $F(x)$ of a discrete random variable $X$ with probability distribution $f(x)$ is:
