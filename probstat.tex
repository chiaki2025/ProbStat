...
\end{enumerate}
This problem will be covered in detail in \textbf{Chapter 4: Discrete Random Variables} and \textbf{Chapter 5: Continuous Random Variables}, but if you do have experience with random variables, you can try solving it!
...
Coin tossing is a typical example of \textbf{Bernoulli trial}, and the experiment tossing unfair coins is a \textbf{Bernoulli process}. The probability $P(A)$ can be calculated by using \textbf{Binomial distribution} formula. You do not have to worry about these terms, we will cover them in \textbf{Chapter 4: Discrete Random Variables} very carefully.
\\ \\ \\
...
\begin{figure}[h]
	...
	\label{Figure 28}
\end{figure}
\\Now you should review \textbf{Chapter 4: Some Discrete Distributions} and try applying this theorem.
...
\begin{center}
	...
	\end{tabular}
\end{center}
Of course ensuring everything correct is very \textbf{impossible}, so \textbf{trading-off} our unknown distribution $f(x)$ with \textbfP{normal assumption} is a promising idea; and we can determine the \textbf{interval} of $\mu$ or $\sigma^{2}$ may be fallen in with certainty up to $95\%$. As I mentioned before, because of our small sample size, we can not use Z-test; t-test and Chi-test are  the better choices in our situation.
...
\begin{figure}[h]
	...
	\label{Figure 36}
\end{figure}
\\ Let $t_{\alpha}$ represent the $t$ value above which we find an area of $\alpha$. Similarly to others test statistics, same meaning can also be applied for $z_{\alpha}$ and $\chi^{2}_{\alpha}$. So $t_{0.025}$ and $-t_{0.025}$ are the values that we find the \textbf{red area} above is $0.025$ (because $\mathscr{T}(t;v)$ graph is symmetrical). The \textbf{white area} that is bounded by two ends is $0.95$:
$$-t_{0.025}<T<t_{0.025}$$
