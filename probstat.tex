...
\begin{figure}[h]
	...
	\label{Figure 39}
\end{figure}
Similarly, you can establish lower one-sided bounds interval with some slightly changes:
...
\\ The first step is always making two hypotheses $H_{0}$ and $H_{1}$; relatively they are logcical complements, so if our evidence supports for $H_{0}$, then $H_{1}$ is more likely to be wrong, and vice versa.
\begin{equation*}
	\begin{split}
		H_{0}&:\mu=4.033\\
		H_{1}&:\mu<4.033\\
...
When we normalize the statistic quantity $\overline{X}$ with the population mean $\mu=4.033$, we did assume $H_{0}$ is \textbf{true} (or to be more precise, did not have any evidence to refute it). Now before understading the meaning of $\alpha$ as \textbf{probability of commiting a type I error}, we will visualize it first. In the previous chapter, coefficient $\alpha$ is defined as the red area, and $100(1-\alpha)\%$ is the \textbf{confidence interval}.
\begin{figure}[h]
	\centering
...
Intuitively, $t$ and $-t_{0.1}$ values are very \textbf{close}; of course this evidence can not refute the null hypothesis $H_{0}$, but it is \textbf{not a convincing evidence} (or strong evidence). The value of $t$ is very near the \textbf{critical region}. Nonrejection of $H_{0}$ can be acceptable, but this decision might not be so convincible. How about we \textbf{do reject $H_{0}$}, and compute the probability of this decision is wrong? The computed value is called as the \textbf{P-value}.
...
\begin{enumerate}
	...
	\item Choose a test statistic and establish the critical region based on $\alpha$.
	      \\ As you can see here, our sample size is very large $n=100$, and we are working with \textbf{porportion} quantity, so Z-test is selected. To avoid confusion, you should sketch a simple graph here.
	      \begin{figure}[h]
		      \centering
...
$\beta_{0}$ is the \textbf{intercept} and $\beta_{1}$ is the \textbf{slope}. But obviously, in many cases, we can not find the exact values for $\beta_{0}$ and $\beta_{1}$ for a given data set $D=\{(x_{i},y_{i})_{i=1}^{n}\}$. Now we might try padding a small quanitity unit, called \textbf{error} to the ideal model:
	...
	\end{theorem}
	It very hard to prove this theorem rigorously, but the reason we divide $n-2$ is just there are $2$ random variables in SSE function. Finally, $\beta_{1}$ and $\beta_{0}$ can be estimated by t-test with $v=n-2$ degrees of freedom:
