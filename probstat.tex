...
$$\int_{-\infty}^{+\infty}f(x)dx=1$$
To avoid the constant $c$ ($c$ is just a scaling factor), we consider the special case where $c=1$ and calculate its integral value:
...
For convenience, we usually choose $c=\frac{1}{2}$ to be standard scaling factor and define:
...
\begin{figure}[h]
	...
	\caption{The pdf of $\mathscr{N}(z;0,1)$}
	\label{Figure 26}
\end{figure}
...
\begin{figure}[h]
	...
	\label{Figure 27}
\end{figure}
As you can see the general normal distribution function can be formed in two steps: shifting the standard normal distribution curve and scaling it. Now the roles of the \textbf{mean} $\mu_{X}$ and the \textbf{standard deviation} $\sigma_{X}$ are very clear. The \textbf{mean} acts as the \textbf{axis of symmetry}, while the \textbf{standard deviation} acts as the scaling factor of the general normal distribution graph. Formally, we define:
