...
Intuitively, you can certainly conclude that $X$ and $Y$ are \textbf{statistically independent} since coins and bottle cap tossing process do not affect each other. So the \textbf{joint probability distribution} of them is:
...
\begin{proof} Proving this theorem above is quiet difficult and requires knowledge of \textbf{Linear Algebra}. I recommend skipping this proof and moving on to the next section to learn some \textbf{useful theorems} first, and then back here later.
	\\ In the \textbf{random variables vector space} sharing the joint probability distribution function, we now define the \textbf{inner product} is:
	$$\left<X,Y\right>=E(XY)$$
	...
	\end{equation*}
	Since $k$ is an arbitrary value in $\mathbb{R}$, so we can conclude that if and only if $|\rho_{XY}|=1$, then $Y=aX+b\;(a,b\in\mathbb{R})$ or equivalently they have a linear relationship.
\end{proof}
\begin{corollary}
	If $\rho_{XY}=0$, then $X$ and $Y$ are \textbf{statistically independent}.
\end{corollary}
\begin{proof} $\rho_{XY}=0\Leftrightarrow \sigma_{XY}=E(XY)-\mu_{X}\mu_{Y}=0\Leftrightarrow E(XY)=\mu_{X}\mu_{Y}$
...
\end{theorem}
\begin{proof} Applying directy the definition of the \textbf{expected value}:
	\begin{equation*}
		\begin{split}
...
\begin{theorem}
	If $X$ and $Y$ are two \textbf{independent random variables} with the joint probability distribution function $f(x,y)$ with three constants $a,b,c$, then: $$\sigma^2_{aX+bY+c}=a^2\sigma^2_{X}+b^2\sigma^2_{Y}$$
\end{theorem}
...
\end{theorem}
\begin{proof} Applying Markov's inequality with $(X-\mu_{X})^2$ as non-negative random variable:
	\begin{equation*}
		\begin{split}
...
This result is very important, especially in \textbf{case of continuous random variable}, the range of $X$ usually extends to infinity; it's shows us that the first and last ends of any pdf are \textbf{bounded}. In general, the shape of a valid pdf graph is always \textbf{flattened} at both ends according to the inequality above.
\begin{figure}[h]
	\centering
