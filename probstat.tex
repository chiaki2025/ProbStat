...
\\ In the previous chapter, the coefficient $\alpha$ does not have a name, but now you can understand very deeply its meaning in hypothesis testing problems. The probability of \textbf{both} $H_{0}$ is correct and edge case happening is $\alpha\%$; it is a very small number, but \textbf{never can be zero}. So if our conclusion is "Refute $H_{0}$" when we see an edge case, the probability of wrong decision is $\alpha\%$. For that reason, quantity $\alpha\%$ is said to be a \textbf{level of significance}, and can be understood as the probability of \textbf{rejecting hypothesis} $H_{0}$ decision is wrong when we encounter an edge case.
